...
class SpotlightElementResolver extends AbstractCmsElementResolver
{
    ...
    {
        return 'scalecommerce-vo-spotlight';
    }
    ...
    {
        $struct = new SpotlightStruct();
        $slot->setData($struct);

        $config = $slot->getFieldConfig();
        $string = fn (string $key): ?string => is_string($config->get($key)?->getValue()) ? $config->get($key)->getValue() : null;

        $struct->setPresentation(in_array($config->get('presentation')?->getValue(), ['facade', 'lightbox', 'direct'], true) ? $config->get('presentation')->getValue() : 'lightbox');
        $struct->setEyebrow($string('eyebrow'));
        $struct->setHeadline($string('headline'));
        $struct->setCaption($string('caption'));

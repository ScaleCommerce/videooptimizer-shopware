import template from './scale-video-optimizer-list.html.twig';

const { Component, Mixin } = Shopware;

Component.register('scale-video-optimizer-list', {
    template,
    ...
    methods: {
        ...
        async loadVideos() {
            ...
            this.isLoading = true;
            try {
                const response = await this.scaleVideoOptimizerApiService.getVideos(this.selectedLibraryId);
        ...
        async onCreateLibrary(name) {
            ...
            try {
                await this.scaleVideoOptimizerApiService.createLibrary({ name });
                await this.loadLibraries();
                this.createNotificationSuccess({ message: this.$tc('scale-video-optimizer.list.libraryCreated') });
            } catch (error) {
                this.createNotificationError({ message: this._errorText(error) });
            }
        },
        ...
        async onDeleteLibrary(id) {
            try {
                await this.scaleVideoOptimizerApiService.deleteLibrary(id);
        ...
        async onDeleteVideo(uuid) {
            try {
                await this.scaleVideoOptimizerApiService.deleteVideo(uuid);
                await this.loadVideos();
            } catch (error) {
                this.createNotificationError({ message: this._errorText(error) });
            }
        ...
        async onRenameVideo(uuid, title) {
            ...
            try {
                await this.scaleVideoOptimizerApiService.updateVideo(uuid, { title });
                await this.loadVideos();
            } catch (error) {
                this.createNotificationError({ message: this._errorText(error) });
            }
